  ...
  <Modal :value="dialogShow" :title="dialogTitle" :width="700" @on-cancel="cancel" :styles="{bottom:'20px',top: '50px'}">
    <div style="height: 600px;overflow-y: scroll;overflow-x: hidden">
      <Form ref="createFortesting" :model="formItem" :label-width="80" :rules="ruleCustom">
        ...
        <!--<FormItem label="模块" prop="ProjectModuleID">-->
          <!--<Select v-model="formItem.ProjectModuleID" :filterable="true" placeholder="模块">-->
            <!--<Option v-for="module in projectModules" :key="module.id" :value="module.id">{{ module.Name }}</Option>-->
          <!--</Select>-->
        <!--</FormItem>-->
        ...
        <FormItem label="分支" prop="Branch">
          <Input v-model="formItem.Branch" placeholder="代码分之"/>
        </FormItem>
        <FormItem label="提测内容:" prop="TestingFeature">
          <vue-editor v-model="formItem.TestingFeature" :editorToolbar="editorToolBar"
                      @text-change="commitContentChange" placeholder="提测内容"></vue-editor>
        </FormItem>
        ...
        loadForTesting: function () {
          ...
          if (this.formItem.id !== 0) {
            ...
            initPromise.then(function (initData) {
              ...
              defaultTask.TestingFeature = initData.TestingFeature.replace('div','p')
...
//      defaultProjectID: function (value) {
//        this.loadProjectModules(value[value.length-1])
//      },
